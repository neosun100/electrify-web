...
async function checkApp(appRoot, inputOptions) {
    ...
    if (inputOptions.out !== undefined) {
        expect(path.join(inputOptions.out, `npm-${inputOptions.platform}-${arch}`)).toBe(appRoot);
    }
    ...
    // Test name inferring
    expect(nativefierConfig === null || nativefierConfig === void 0 ? void 0 : nativefierConfig.name).toBe('npm');
    // Test icon writing
