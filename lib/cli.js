...
require("./patches/extractZipNodeCompat");
...
if (require.main === module) {
    ...
    // list 命令 - 列出已安装的应用
        ...
        const apps = (0, registry_1.listApps)();
        if (apps.length === 0) {
    ...
    // remove 命令 - 卸载应用
        ...
        if (!appName) {
            ...
            console.log('\nInstalled apps:');
            (0, registry_1.listApps)().forEach((app) => console.log(`  - ${app.name}`));
            process.exit(1);
        }
        console.log(`\n🗑️  Uninstalling "${appName}"...`);
        const result = (0, registry_1.uninstallApp)(appName, purge);
        if (result.success) {
