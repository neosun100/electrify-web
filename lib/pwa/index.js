...
exports.fetchPWAInfo = fetchPWAInfo;
exports.getBestIconFromManifest = getBestIconFromManifest;
exports.pwaDisplayToOptions = pwaDisplayToOptions;
exports.pwaInfoToOptions = pwaInfoToOptions;
...
function extractAttr(html, tagPattern, attr) {
    ...
    if (match) {
        ...
        return attrMatch === null || attrMatch === void 0 ? void 0 : attrMatch[1];
    }
    return undefined;
}
/**
 ...
 * 从 HTML 中提取 PWA 相关信息
 */
function extractPWAInfoFromHtml(html, baseUrl) {
    const info = {};
    // 主题色
 ...
 * 获取网站的 PWA 信息
 */
async function fetchPWAInfo(url) {
    const info = {};
    try {
        // 获取 HTML
        ...
        if (manifestUrl) {
            ...
            try {
                const manifestResponse = await axios_1.default.get(manifestUrl, { timeout: 5000 });
                info.manifest = manifestResponse.data;
 ...
 * 从 PWA 信息中获取最佳图标 URL
 */
function getBestIconFromManifest(manifest, baseUrl) {
    ...
    const sortedIcons = [...manifest.icons].sort((a, b) => {
        var _a, _b;
        const sizeA = parseInt(((_a = a.sizes) === null || _a === void 0 ? void 0 : _a.split('x')[0]) || '0', 10);
        const sizeB = parseInt(((_b = b.sizes) === null || _b === void 0 ? void 0 : _b.split('x')[0]) || '0', 10);
        return sizeB - sizeA;
    });
    // 优先选择 purpose 为 any 或 maskable 的图标
    const preferredIcon = sortedIcons.find((icon) => !icon.purpose || icon.purpose === 'any' || icon.purpose.includes('any')) || sortedIcons[0];
    try {
 ...
 */
function pwaDisplayToOptions(display) {
    switch (display) {
        case 'fullscreen':
 ...
 */
function pwaInfoToOptions(info, baseUrl) {
    const options = {};
    if (info.manifest) {
        // 名称
