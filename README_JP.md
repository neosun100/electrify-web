...
- 🧹 アプリが手動削除された時、古いレコードを自動クリーンアップ
- 📍 アプリが /Applications に移動された時、パスを自動更新

### v1.1.1 (2025-12-21)

- 🔍 移動されたアプリのスマートパス検出（macOS/Linux対応）

### v1.1.0 (2025-12-21)

- ✨ 新コマンド: `eweb list` - インストール済みアプリ一覧
- ✨ 新コマンド: `eweb remove <name>` - アプリのアンインストール
- 📝 アプリレジストリ: `~/.electrify-web/registry.json`

### v1.0.7 (2025-12-21)

- 🔧 修正: `cli.js` に実行権限を付与し、グローバルインストールをサポート

### v1.0.6 (2025-12-21)

- 💬 改善: アイコン変換失敗時のエラーメッセージを改善

### v1.0.5 (2025-12-21)

- 🔧 修正: macOS `iconutil` に必要なアイコンサイズを修正（無効な64x64を削除）

### v1.0.4 (2025-12-21)

- ✨ 新機能: PWA検出をデフォルトで有効化（`--pwa` がデフォルトでtrue）

### v1.0.3 (2025-12-21)

- 🔧 修正: macOSでImageMagickよりネイティブの `sips` を優先使用

### v1.0.2 (2025-12-21)

- 🔧 修正: ImageMagick v7をサポート（非推奨の `convert` の代わりに `magick` コマンドを使用）
